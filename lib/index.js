...
import { DEFAULT_SCOPES, buildAuthorizationUrl, buildPostPayload, createPkcePair, createState, formatApiError, isAbortError } from "./x-api.js";
...
function requireConfigured(config) {
  if (!config.clientId || !config.redirectUri) throw new XPublisherError("X clientId and exact redirectUri must be configured", "NOT_CONFIGURED");
}
...
export function apply(ctx, rawConfig) {
  ...
  const client = {
    ...
    async exchangeCode(code, verifier, signal) {
      const form = new URLSearchParams({ grant_type: "authorization_code", code, redirect_uri: config.redirectUri, client_id: config.clientId, code_verifier: verifier });
  ...
  function createPending(signal) {
    ...
    const url = buildAuthorizationUrl({ authorizeUrl: config.authorizeUrl, clientId: config.clientId, redirectUri: config.redirectUri, state, codeChallenge: pkce.challenge, scopes: config.scopes });
    let resolvePromise;
    let rejectPromise;
