...
const zh = {
  ...
  callbackUrlHint: "将此地址完整复制到 X Developer Console 的 Callback URI / Redirect URL。",
  copy: "复制",
  copied: "已复制",
...
const en = {
  ...
  callbackUrl: "Callback URL",
  callbackUrlHint: "Copy this exact value into the Callback URI / Redirect URL field in the X Developer Console.",
  copy: "Copy",
  copied: "Copied",
...
function XPublisherSettingsSection({ t }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setCallbackUrl(String(value.redirectUri ?? ""));
  ...
  const openAuthorization = async () => {
    ...
    try {
      if (value !== savedClientId) {
        ...
        setClientId(nextClientId);
        setSavedClientId(nextClientId);
        setCallbackUrl(String(response.redirectUri ?? ""));
      }
